...
const Questns = () => {
    ...
    const Answers = (ansid, quesid, num) => {
        let arr = [];
        let t = true;
        let k = true
            ...
            answers.forEach(item => {
                if (item.question_id == quesid) {
                    ...
                    if (!item.answer_id.includes(ansid)) {
                        arr.push({ question_id: quesid, answer_id: [...item.answer_id, ansid] })
                    } else {
                        let a = []
                        ...
                        arr.push({ question_id: quesid, answer_id: a })
                    }
                } else {
                    arr.push(item)
                }
    ...
    return (<>
        ...
        <div className="fathe_blok">
            <AccountMenu />
            <div className="xx ">
                {loading === true ? <div className="loadin">
                                            ...
                                            {item.vanswers.map((ite, inde) => (
                                                ...
                                                <li>
                                                    <label className="d-flex ">
                                                        <input type={index % 2 === 0 ? "radio" : "checkbox"} name={index % 2 === 0 ? 'answer' + qindex + index : ''} onChange={(e) => Answers(ite.id, item.id, index)} />
                                                        <div>
                                                            <p><WordSave text={ite.answer} /></p>
                                                        </div>
                                                        <span className={index % 2 === 0 ? "checmak" : "checmakk"}></span>
